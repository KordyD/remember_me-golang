...
import (
	...
	"github.com/kordyd/remember_me-golang/sso/internal/services/auth/mocks"
...
func TestAuthServiceLogin_HappyPath(t *testing.T) {
	...
	mockedUserStorage.On("SaveUser", ctx, email, mock.AnythingOfType("[]uint8")).Return(id, nil)
	...
	authService := auth.New(slog.Default(), mockedUserStorage, mockedAppStorage, tokenTTL)
	userId, err := authService.Register(ctx, email, password)
	require.NoError(t, err)
	assert.NotEmpty(t, userId)
